...
var LastFmRequest = module.exports = function(lastfm, method, params) {
  ...
  if (lastfm.cache) {
      var key = createCacheKey(requestParams);
      lastfm.cache.get(key);
  }
  sendRequest(lastfm.host, lastfm.url, requestParams);

  function sendRequest(host, url, requestParams) {
  ...
  function createSignature(params, secret) {
    var sig = stringifyParameters(params, false);
    sig += secret;
    return crypto.createHash("md5").update(sig, "utf8").digest("hex");
  }

  function createCacheKey(params) {
    return crypto.createHash('md5').update(stringifyParameters(requestParams, true), 'utf8').digest('hex');
  }

  function stringifyParameters(params, includeFormat) {
    var stringified = '';
